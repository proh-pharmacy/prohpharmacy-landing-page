...
export function HeroIllustration() {
  ...
  // When image loads, it appears IMMEDIATELY.
  // We only delay the unmounting of the skeleton so it cross-fades away gracefully underneath.
  useEffect(() => {
    if (imageLoaded) {
  ...
  return (
    <div className="relative w-full max-w-[560px] aspect-[1200/800] flex items-center justify-center lg:justify-start select-none">
      {/* Skeleton placeholder: stays mounted and cross-fades out smoothly when image appears */}
      {showSkeleton && (
        <div
          className={`w-full flex items-center justify-center transition-opacity duration-700 transform translate-x-[50px] scale-[0.97] ${
            imageLoaded
      ...
      {/* Real image: appears IMMEDIATELY when loaded (zero artificial delay) and rolls into place */}
      <div
        className={`w-full flex items-center justify-center absolute inset-0 ${
          imageLoaded ? "rider-entrance-animation" : "opacity-0 pointer-events-none"
        }`}
      >
        <Image
